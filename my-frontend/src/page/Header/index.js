...
import BasicExample from "../MenuHeader";
...
const Header = ({setclickSearch}) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:1337/api/the-loais').then((res) => {
      ...
      const returnidloai = res?.data?.data?.map(
        (loai) =>loai.id
      )
      setidloai(returnidloai)
  ...
  const renderItem = () =>
    theloai?.map((item, index) => (
      <li  key={index}>
        <Link onClick={()=>setTheLoai(index+1)} to={"/theloai/"+idloai[index]+'/1'}>{item}</Link>
      </li>
    ));
